...
def init_params_pseudo_encoder(params):
    enc = ['Wemb',
           'encoder_U',
           'encoder_W',
           'encoder_r_Wx',
    ...
    for kk, vv in params.iteritems():
        if kk in enc:
            new_params['pseudo_'+kk] = params[kk].astype(floatX, copy=False)
        new_params[kk] = params[kk].astype(floatX, copy=False)
...
def load_params(path, params, with_prefix=''):
    ...
    # load disciminator and pseudo encoder parameters
    for kk, vv in pp.iteritems():
       if kk.startswith('discriminator_') or kk.startswith('pseudo_'):
          new_params[kk] = pp[kk].astype(floatX, copy=False)
